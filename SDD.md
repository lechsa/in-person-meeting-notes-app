...
| Audio Recording | `expo-av` | First-party Expo support; background audio capabilities |
...
**Behavior:**
- On **Record tap**: Request mic permission (if needed) → Create meeting record (status: `recording`) → Start `expo-av` recording → Start duration timer
...
interface AudioService {
  ...
  getRecordingStatus(): Promise<Audio.RecordingStatus>;
}
```

**Implementation details:**
- Uses `expo-av` `Audio.Recording` API
- Configures `Audio.setAudioModeAsync()` for background recording:
  ```typescript
  ...
  ```
- Recording preset: `Audio.RecordingOptionsPresets.HIGH_QUALITY` (WAV/M4A)
...
class SummarizerService:
    async def summarize(self, transcript: str) -> str:
        ...
        MVP: Returns truncated transcript as summary.
        Production: Integrate LLM (OpenAI, Claude, etc.)
        """
...
**AVAudioSession configuration** (handled at runtime in audio service):
```typescript
...
- `staysActiveInBackground: true` on the `AVAudioSession` prevents iOS from suspending the recording
...
Audio.setOnAudioFocusChanged(({ isFocusLost, isPlaying }) => {
  if (isFocusLost && currentRecording) {
    // Pause recording, store timestamp
    currentRecording.pauseAsync();
  } else if (!isFocusLost && currentRecording) {
